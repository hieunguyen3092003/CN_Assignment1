...
def get_local_ip(interface=None):
    # Nếu không có giao diện được chỉ định, tìm giao diện mạng mặc định
    if interface is None:
        if platform.system() == 'Darwin':  # macOS
            interface = 'en0'  # Thường là en0 trên macOS
        else:  # Linux
            interface = 'wlo1'  # Hoặc eth0 tùy vào máy bạn
    ...
    try:
        ...
        if match:
            ...
            print(f"IP address not found for interface {interface}")
            return None
    except subprocess.CalledProcessError as e:
        print(f"Error running ifconfig on {interface}: {e}")
        return None
